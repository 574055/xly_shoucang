...
// uint8_t RFID_scan(uint8_t result[NUMBEROFBYTES]) {
// //print RFID reply
//   //Serial1.listen();
...
//   uint16_t i = 0;
//   uint8_t tags = 0; 
//   //scanTimeOut = millis(); //reset timer
//   //while (millis() - scanTimeOut < 1000){
...
// //        return tags;
// //      }
//       //scanTimeOut = millis() - scanTimeOut;
//       //Serial.println(scanTimeOut);
//     }
//   //}
//   return tags;
// }
...
String buildData(String id, String timestamp, String weight, String unitinfo) { //组建发送的json字符串
  String payload = "{\"unitinfo\":\"testxtray6\",\"data\":{\"weight\":\"";
  payload += weight;
  payload += "\",\"uids\":[";
  payload += id;
  payload += String("]}}");
...
void weightscaleInit(){
  ...
  scale.set_scale(-225.f);  // 開始取得比例參數                      // this value is obtained by calibrating the scale with known weights; see the README for details
  scale.tare();//////////程序崩坏点！！！！！！！
