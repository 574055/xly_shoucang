...
// HX711
float calibration_factor = 22900; //50kg->22900, 5kg-->250000
float output;
int readIndex;
float total = 0;
float average = 0;
...
// Weight scale
  ...
  debugln("TOTAL:");
  Serial.println((String) "w" + ave);       // send weight to GUI
...
void wifiInit()
{
  ...
  while (status != WL_CONNECTED)
  {
    ...
    display.setCursor(0, 0);
    display.println((String) "Connecting to: " + e_data.SSID);
