    ...
    while (!finished_scan) {
        while (Serial1.available()) {
            ...
            if (result[i] == 0x7e) {
                ...
                // if (i < 8)
                if ((i == 23) == (i == 19)) { // if not enough characters, skipp this tag
                    i = 0;
...
{
    static char result[2 * NUMBEROFBYTES
                       + 1]; // Note, there needs to be 1 extra space for this to work as snprintf null terminates.
    char* myPtr = &result[0];
    for (uint8_t i = 0; i < 2; i++) { // loop the tag id
        snprintf(myPtr, 3, "%02x",
                 HexArr[5 + i]); // start from 12th to 16th and add 20 per tag scanned //convert a byte to character
                                 // string, and save 2 characters (+null) to charArr;
        myPtr += 2; // increment the pointer by two characters in charArr so that next time the null from the previous
                    // go is overwritten.
    }
...
String getTagfromHEX(uint8_t HexArr[], uint8_t nTags)
{
    ...
    static char result[2 * NUMBEROFBYTES + 1] = {
        0}; // Note there needs to be 1 extra space for this to work as snprintf null terminates.
    char* myPtr = &result[0];
    uint16_t j = 0;
    uint8_t tags = 0;
    ...
    while (j < NUMBEROFBYTES && tags < nTags) { // loop through the whole HexArr till all Tags are checked
        if (HexArr[j] == 0x7e) {
            ...
            for (uint8_t i = 0; i < 4; i++) { // loop the tag id
                ...
                snprintf(myPtr, 3, "%02x",
                         HexArr[j]); // start from 12th to 16th and add 20 per tag scanned //convert a byte to character
                                     // string, and save 2 characters (+null) to charArr;
                myPtr += 2; // increment the pointer by two characters in charArr so that next time the null from the
                            // previous go is overwritten.
                j++;
            }
            j += 7;
            // Serial.println((String)"j: " + j);
...
String keepIndividualsOfMultiScan(String scan, uint8_t* ntotalTags)
{
    ...
    for (uint8_t i = 0; i < *ntotalTags; i++) {
        if (scan.substring(8 * i, 8 + 8 * i) != "bb01ff00") { // do the checking only if there is a valid tag
            ...
            bool alreadyHere = false;
            while (j < nTags) {
                // Serial.println((String)"Compare: " + scan.substring(8 * i ,8 + 8 * i) + "with: " + result.substring(8
                // * j ,8 + 8 * j));
